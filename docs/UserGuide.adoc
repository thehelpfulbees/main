...
=== Showing all persons in a tag: `show`

Finds all persons in the tag with the name matching the given keyword. +
Format: `find KEYWORD`

****
* Only persons in the tag matching the entered keyword exactly will be shown, so searching `friend` will not show persons with the tag `friends`
* The search is case sensitive, so searching for `Friends` will not show persons with the tag `friends`
* The command only supports searching for one keyword at a time, so searching for `friends neighbours` will yield neither `friends` nor `neighbours`
****

Examples:

* `show friends` +
Returns all persons tagged as `friends`
